...
class JuridicPersonRepository:
    ...
    def withdraw_money(self, juridic_person_id: int, quantify: float):
        try:
            daily_limit = 5000.00
            balance = self.check_balance(juridic_person_id)
            ...
            self.update_balance(juridic_person_id, new_balance)
            return f"Saque de R$ {quantify} realizado com sucesso. Saldo atualizado: R$ {new_balance}"
        except Exception as exception:
            raise exception

    def check_balance(self, juridic_person_id: int):
        with self.__db_connection as database:
            try:
                result = (
                    database.session
                    .query(JuridicPersonTable)
                    .filter(JuridicPersonTable.id == juridic_person_id)
                    .first()
                )
                return result.saldo
            except NoResultFound:
    ...
    def update_balance(self, juridic_person_id: int, new_balance: float) -> None:
        with self.__db_connection as database:
            try:
                result = (
                    database.session
    ...
    def view_statement(self, juridic_person_id):
        with self.__db_connection as database:
            try:
                result = (
                    database.session
